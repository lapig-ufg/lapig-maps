...
def trend(layerId, startYear, endYear, interpolation, groupData, timeChange, timeChangeUnits, geoJsonGeometry):
    ...
    clippedValues = bfast_utils.clipValuesByYear(
        startYear, endYear, timeserieData["values"], datesStr)
    ...
    interpolationIndex, interpolationPos = next(((i, item['position']) for i, item in enumerate(
        timeserieData["series"]) if item['id'] == interpolation), (-1, -1))

    # Inicializa preparação do objeto de resposta
    series = [{'id': 'Bfast', 'label': 'BFAST',
               'position': 1, 'type': 'trend'}]
    ...
    if interpolationPos != -1:
        ...
        series.append({'id': timeserieData["series"][interpolationIndex]["id"],
                       'label': timeserieData["series"][interpolationIndex]["label"], 'position': 2, 'type': 'filter'})

    else:
        series.append({'id': 'original', 'label': 'Valores Originais',
                       'position': 2, 'type': 'original'})
    ...
    result = []
    if bfastIndex != -1:
        result = filters[bfastIndex].run(
            values, None, None, minimalSegmentSize, frequency, startDate, endDate)
    else:
