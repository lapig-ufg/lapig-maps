...
var app = new gxp.LapigViewer({
		...
		sources: {
				ows: {
						url: '/ows/', // http://localhost:5000/ows?SERVICE=WMS&VERSION=1.1.0&request=GetCapabilities
						title: "LAPIG-OWS"
				}
				, mapquest: {
		...
		listeners: {
			'ready': function() {
				...
				var wmsLayerToTop = function() {
					var baseLayers = [];
					var wmsLayers = []

					mapPanel.map.layers.forEach(function(layer) {
					...
					baseLayers.forEach(function(layer) {
						mapPanel.map.setLayerZIndex(layer, index++);
					})
